...
exports.runScenario6 = async (req, res) => {
  const logs = []
  const connA = await mysql.createConnection(dbConfig)
  const connB = await mysql.createConnection(dbConfig)

  try {
    logs.push('--- Scenario 6: Non-Repeatable Read (READ COMMITTED) ---')

    // Session B: Start a transaction with READ COMMITTED isolation
    await connB.execute('SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED')
    await connB.beginTransaction()
    logs.push('B: Set isolation to READ COMMITTED and started transaction.')

    // Session B: First read
    const [rows1] = await connB.execute('SELECT Request_ID, Status FROM VERIFICATION_REQUEST WHERE Request_ID = 5')
    const firstRead = rows1[0]?.Status || 'N/A'
    logs.push(`B: First read of Request #5 → Status = "${firstRead}"`)

    // Session A: Update and COMMIT
    await connA.beginTransaction()
    await connA.execute("UPDATE VERIFICATION_REQUEST SET Status = 'In_Progress' WHERE Request_ID = 5")
    await connA.commit()
    logs.push('A: Updated Request #5 to "In_Progress" and COMMITTED.')

    // Session B: Second read (same transaction) — value has changed!
    const [rows2] = await connB.execute('SELECT Request_ID, Status FROM VERIFICATION_REQUEST WHERE Request_ID = 5')
    const secondRead = rows2[0]?.Status || 'N/A'
    logs.push(`B: Second read of Request #5 → Status = "${secondRead}" <--- NON-REPEATABLE READ`)

    if (firstRead !== secondRead) {
      logs.push(`Result: Value changed from "${firstRead}" to "${secondRead}" within the SAME transaction.`)
      logs.push('This demonstrates a Non-Repeatable Read anomaly.')
    } else {
      logs.push('Note: Values were identical (the row may already have had that status).')
    }

    await connB.commit()
    logs.push('B: Transaction committed.')

    // Restore the original status
    await connA.execute("UPDATE VERIFICATION_REQUEST SET Status = 'Pending' WHERE Request_ID = 5")
    logs.push('Cleanup: Restored Request #5 to original status.')
